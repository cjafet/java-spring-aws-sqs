...
class VideoEntityTest {
    ...
    void testVideoEntity_AllFields() {
        ...
        videoEntity.setVideoUrl("https://example.com/video.mp4");
        ...
        assertEquals("https://example.com/video.mp4", videoEntity.getVideoUrl());
        assertEquals(100L, videoEntity.getUserId());
    }

    @Test
    void testUserVideosDTO_AllFields() {
        UserVideosDTO userVideosDTO = new UserVideosDTO(2L, VideoStatus.COMPLETED, "https://example.com/processed.mp4", 200L);

        assertEquals(2L, userVideosDTO.getId());
        assertEquals(VideoStatus.COMPLETED, userVideosDTO.getVideoStatus());
        assertEquals("https://example.com/processed.mp4", userVideosDTO.getVideoUrl());
